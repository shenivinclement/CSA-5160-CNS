...
def permute(input_val, table, table_size, input_size):
    output = 0
    for i in range(table_size):
        bit_pos = input_size - table[i]
        bit = (input_val >> bit_pos) & 1
        output |= (bit << (table_size - 1 - i))
    return output

def generate_subkeys(key):
    key_pc1 = permute(key, PC1_TABLE, 56, 64)
    C = (key_pc1 >> 28) & 0x0FFFFFFF
    D = key_pc1 & 0x0FFFFFFF
    subkeys = []
    for i in range(16):
        ...
        combined = (C << 28) | D
        subkeys.append(permute(combined, PC2_TABLE, 48, 56))
    return subkeys

def feistel_function(R, subkey):
    expanded = permute(R, E_TABLE, 48, 32)
    ...
    return permute(sbox_output, P_TABLE, 32, 32)

def process_block(block, subkeys):
    ip_block = permute(block, IP_TABLE, 64, 64)
    L = (ip_block >> 32) & 0xFFFFFFFF
    R = ip_block & 0xFFFFFFFF
    for i in range(16):
